...
The [@tbl-rf] are the prediction results obtained through Random Forest. Compared to a linear model, this result may be more accurate; however, in the overall study, this method is only provided and will not be the main subject of research.
...
The [@tbl-model-state] provides a more detailed numerical representation of the predicted support percentages and projected winners for each state. @tbl-model-state complements @fig-map, allowing for a more specific view of Trump and Harris performance in each state.
